...
namespace Kamgam.UVEditor
{
    public partial class UVEditorWindow : EditorWindow
    {
        ...
        [System.NonSerialized] protected List<VisualElement> _gridBottomTools; // All elements with class grid-bottom-toolbar
        [System.NonSerialized] protected TextField _gridWidthInput;
        [System.NonSerialized] protected TextField _gridHeightInput;
        [System.NonSerialized] protected Vector2 _gridSize = new Vector2(0.1f, 0.1f);
        
        [System.NonSerialized] protected List<VisualElement> _vertexPosBottomTools; // All elements with class vertexpos-bottom-toolbar
        [System.NonSerialized] protected TextField _vertexPosUInput;
        [System.NonSerialized] protected TextField _vertexPosVInput;
        ...
        public void CreateGUI()
        {
            ...
            _gridBottomTools = _root.Query(className: "grid-bottom-toolbar").ToList();
            _gridWidthInput = _root.Q<TextField>("GridWidth");
            _gridWidthInput.RegisterValueChangedCallback(onGridWidthInputChanged);
            _gridHeightInput = _root.Q<TextField>("GridHeight");
            _gridHeightInput.RegisterValueChangedCallback(onGridHeightInputChanged);
            foreach (var ve in _gridBottomTools)
            {
                ve.style.display = DisplayStyle.None;
            }
            
            _vertexPosBottomTools = _root.Query(className: "vertexpos-bottom-toolbar").ToList();
            _vertexPosUInput = _root.Q<TextField>("VertexPosU");
            _vertexPosUInput.RegisterValueChangedCallback(onVertexPosUInputChanged);
            _vertexPosVInput = _root.Q<TextField>("VertexPosV");
            _vertexPosVInput.RegisterValueChangedCallback(onVertexPosVInputChanged);
            foreach (var ve in _vertexPosBottomTools)
            {
                ve.style.display = DisplayStyle.None;
            }
        ...
        private void onGridWidthInputChanged(ChangeEvent<string> evt)
        {
            if (float.TryParse(evt.newValue, NumberStyles.Any, CultureInfo.InvariantCulture, out var value))
                _gridSize.x = value;
        }
        
        private void onGridHeightInputChanged(ChangeEvent<string> evt)
        {
            if (float.TryParse(evt.newValue, NumberStyles.Any, CultureInfo.InvariantCulture, out var value))
                _gridSize.y = value;
        }
        
        private void onVertexPosUInputChanged(ChangeEvent<string> evt)
        {
            if (float.TryParse(evt.newValue, NumberStyles.Any, CultureInfo.InvariantCulture, out var value) && !_selectedVertices.IsNullOrEmpty())
            {
                startMovingUVs(null);
                moveSelectedU(value);
                stopMovingUVs();
        ...
        private void onVertexPosVInputChanged(ChangeEvent<string> evt)
        {
            if (float.TryParse(evt.newValue, NumberStyles.Any, CultureInfo.InvariantCulture, out var value) && !_selectedVertices.IsNullOrEmpty())
            {
                startMovingUVs(null);
        ...
        private void updateVertexPosInputs()
        {
            if (_selectedVertices.IsNullOrEmpty())
            {
                _vertexPosUInput.SetEnabled(false);
                _vertexPosVInput.SetEnabled(false);
            }
            else
            {
                _vertexPosUInput.SetEnabled(true);
                _vertexPosVInput.SetEnabled(true);

                Bounds size = getSelectedVerticesBoundsInUVs();
                _vertexPosUInput.SetValueWithoutNotify(size.center.x.ToString(CultureInfo.InvariantCulture));
                _vertexPosVInput.SetValueWithoutNotify(size.center.y.ToString(CultureInfo.InvariantCulture));
            }
        }

        private Bounds getSelectedVerticesBoundsInUVs()
        {
            var uvs = getUVWorkingCopyForSelection(_uvChannel);
            
            var size = new Bounds();
            size.center = new Vector3(-999_999f, 0f, 0f);
            foreach (var i in _selectedVertices)
            {
                // First
                if (size.center.x < -999_998f)
                {
                    size.center = new Vector3(uvs[i].x, uvs[i].y, 0f);
                    size.extents = Vector3.zero;
                }
                else
                {
                    // other
                    size.Encapsulate(uvs[i]);
                }
            }
        ...
        private void UpdateWindowContent()
        {
            ...
            foreach (var gridBottomTool in _gridBottomTools)
            {
                gridBottomTool.style.display = showLivePreview ? DisplayStyle.Flex : DisplayStyle.None;
            }
            foreach (var vertexPosTool in _vertexPosBottomTools)
            {
                vertexPosTool.style.display = showLivePreview ? DisplayStyle.Flex : DisplayStyle.None;
            }
        ...
        private void updateSelectionRect()
        {
            ...
            _selRectInUVs.xMin = 10f; // not zero and one as that would lead to a visual glitch where the yellow rect would always stay inside the 0-1 range.
            _selRectInUVs.yMin = 10f;
            _selRectInUVs.xMax = -10f;
            _selRectInUVs.yMax = -10f;
